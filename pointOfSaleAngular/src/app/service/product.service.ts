...
export class ProductService {
  ...
  private baseUrl: string = 'http://localhost:8085/api/product';
...
// findProductByCategoryId(categoryId: number): Observable<ProductModule[]> {
//   return this.httpClient.get<ProductModule[]>(`${this.baseUrl}/category/${categoryId}`);
// }
...
findProductByCategoryName(categoryName: string): Observable<ProductModule[]> {
  ...
  getAllDhanmondiBrancesProduct(): Observable<ProductModule[]> {
    ...
    // get all brance product  method

    getAllBrancesProduct(): Observable<ProductModule[]> {
  ...
  createProduct(product: ProductModule, image: File): Observable<ProductModule> {
    ...
    formData.append('medicines', image);
...
console.log(headers);
 ...
 updateProduct(id: number, product: ProductModule, image: File): Observable<any> {
  ...
  getProductByCategoryId(categoryId: number): Observable<ProductModule>{
    return this.httpClient.get<ProductModule>(this.baseUrl + '/product/category/' + categoryId);
  }

...
import { faBox, faCalendarAlt, faCartPlus, faDollarSign, faPlus, faSave, faSortNumericDown, faTags, faTrash, faUser, faWarehouse }
  ...
  createSales() {
    ...
    this.salesService.createSalesGulshanBranch(this.sale).subscribe({
      next: res => {
        ...
        this.router.navigate(['gulshaninvoice'], { queryParams: { sale: JSON.stringify(this.sale) } });
      },
      error: error => { console.log(error); }
    });
  }
